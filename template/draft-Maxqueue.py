...
#         #print('max_add_last:',self.max_deque.add_last(0))
#         if x > self.max(): #compare the x with the max value of the maxdeque_list
#             for i in range(self.max_deque.n): #if x is bigger, run in the n-maxdeque,
...
#                 if x > self.max_deque.get(i): #compare with each character
#                     self.max_deque.add(i,x) #if the x is bigger than any value in the list, then add x in that position
#                     while x > self.max_deque.dummy.prev.x:  #start to remove the other element that is smaller than x
#                         self.max_deque.remove_last() #and remove from the last til x become the smallest
#                     break
#
#
